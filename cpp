...
getline(cin, author); books.push_back(Book(nextId, title, author)); 
...
cin.ignore();getline(cin, student->name); 
cout << "Enter student ID: "; 
cin >> student->id; 
books[i].setAvailability(false); 
books[i].setBorrower(student); issuedBooks.push(make_pair(&books[i], student)); 
...
cin >> choice; vector<Book> sortedBooks = books; if (choice == 1) { 
...
cout << "Book Details:" << endl; cout << "ID: " << book.getId() << endl; 
